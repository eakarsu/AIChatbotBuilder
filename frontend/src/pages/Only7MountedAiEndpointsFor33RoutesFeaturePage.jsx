...
export default function Only7MountedAiEndpointsFor33RoutesFeaturePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-only-7-mounted-ai-endpoints-for-33-routes-feature-/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 7 mounted AI endpoints for 33 routes — feature parity gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-7-mounted-ai-endpoints-for-33-routes-feature-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
