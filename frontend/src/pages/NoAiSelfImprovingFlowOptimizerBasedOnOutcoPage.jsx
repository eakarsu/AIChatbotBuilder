...
export default function NoAiSelfImprovingFlowOptimizerBasedOnOutcoPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-ai-self-improving-flow-optimizer-based-on-outco/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No AI self-improving flow optimizer based on outcomes</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-ai-self-improving-flow-optimizer-based-on-outco/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
