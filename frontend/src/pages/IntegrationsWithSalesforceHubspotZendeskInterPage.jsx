...
export default function IntegrationsWithSalesforceHubspotZendeskInterPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-integrations-with-salesforce-hubspot-zendesk-inter/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Integrations with Salesforce, HubSpot, Zendesk, Intercom, and Shopify</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-integrations-with-salesforce-hubspot-zendesk-inter/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
