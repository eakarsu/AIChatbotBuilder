...
export default function NoLiveChatTakeoverUiForHumanAgentsChannelsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-live-chat-takeover-ui-for-human-agents-channels/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No live-chat takeover UI for human agents (channels module exists but no operator console)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-live-chat-takeover-ui-for-human-agents-channels/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
