...
export default function SentimentTriggeredEscalationToHumanAgentsInPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-sentiment-triggered-escalation-to-human-agents-in-/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Sentiment-triggered escalation to human agents in real time</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-sentiment-triggered-escalation-to-human-agents-in-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
