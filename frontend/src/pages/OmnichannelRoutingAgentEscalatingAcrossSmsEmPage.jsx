...
export default function OmnichannelRoutingAgentEscalatingAcrossSmsEmPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-omnichannel-routing-agent-escalating-across-sms-em/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Omnichannel routing agent escalating across SMS, email, chat, and voice with context handoff</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-omnichannel-routing-agent-escalating-across-sms-em/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
