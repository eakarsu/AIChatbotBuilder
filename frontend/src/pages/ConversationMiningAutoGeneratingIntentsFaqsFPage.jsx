...
export default function ConversationMiningAutoGeneratingIntentsFaqsFPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-conversation-mining-auto-generating-intents-faqs-f/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Conversation mining auto-generating intents/FAQs from production logs</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-conversation-mining-auto-generating-intents-faqs-f/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
